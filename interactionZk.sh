...
# Deploy MarkleAirdrop contract
echo "Deploying MarkleAirdrop contract..."
AIRDROP_ADDRESS=$(forge create src/MarkleAirdrop.sol:MarkleAirdrop --rpc-url http://127.0.0.1:8011 --private-key ${DEFAULT_ZKSYNC_LOCAL_KEY} --constructor-args ${ROOT} ${TOKEN_ADDRESS} --legacy --zksync | awk '/Deployed to:/ {print $3}' )
echo "MarkleAirdrop contract deployed at: $AIRDROP_ADDRESS"
